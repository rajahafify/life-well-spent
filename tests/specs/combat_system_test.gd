...
# Spec: CombatSystem — Field combat affects combat HP, not Life.
...
func test_player_attack_damages_enemy_hp_with_minimum_one_damage() -> void:
	...
	assert_eq(100, player["life"], "Field combat should not change Life")
	combat.free()
...
func test_enemy_attack_damages_combat_hp_not_life() -> void:
	var combat = _combat_system()
	if combat == null:
		return
	var enemy := {"attack": 2}
	var player := {"combat_hp": 10, "defense": 0, "life": 100}
	var result: Dictionary = combat.enemy_attack_player(enemy, player)
	assert_eq(2, result["damage"])
	assert_eq(8, result["player_state"]["combat_hp"])
	assert_eq(100, result["player_state"]["life"])
	assert_eq(100, player["life"])
	combat.free()


func test_enemy_attack_reports_player_combat_down() -> void:
	var combat = _combat_system()
	if combat == null:
		return
	var enemy := {"attack": 10}
	var player := {"combat_hp": 3, "defense": 0, "life": 100}
	var result: Dictionary = combat.enemy_attack_player(enemy, player)
	assert_true(result["player_defeated"])
	assert_eq(0, result["player_state"]["combat_hp"])
	assert_eq(100, result["player_state"]["life"])
	combat.free()
