...
- Automated suite: `348 tests, 348 passed, 0 failed`
...
12. Swordsman Guild certification is playable from the Guildmaster after the Forest Guard checkpoint, spends Max Life in three quest-completion steps, unlocks `swordsman_guild`, advances the quest objective to `Enter the Forest.`, and changes the Forest Guard and Forest gate to a `To be continued` endpoint without reverting the objective.
