...
- Automated suite: `347 tests, 347 passed, 0 failed`
...
12. Swordsman Guild certification is playable from the Guildmaster after the Forest Guard checkpoint, spends Max Life in three quest-completion steps, unlocks `swordsman_guild`, advances the quest objective to `Enter the Forest.`, and changes the Forest gate to a `To be continued` endpoint.
