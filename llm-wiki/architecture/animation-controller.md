...
## Overview
LPC spritesheet animation state machine. Pure model — no Node references. Handles frame coordinate calculation for idle frame cycling and walking frame advance while preserving facing direction.

## Design
- Uses 832px LPC atlas: 13 columns × 21 rows, 64×64 cells
- `frame_coords: Vector2i` — column/row pair for `Sprite2D.frame_coords`
- Idle: cycles frame columns 0-3 every 0.5s while preserving direction row
...
## Test Coverage
20 tests in `tests/specs/animation_controller_test.gd`:
