...
## Overview
LPC spritesheet animation state machine. Pure model — no Node references. Handles frame coordinate calculation for idle (direction cycling) and walking (frame advance) states.

## Design
- Uses 832px LPC atlas: 13 columns × 21 rows, 64×64 cells
- `frame_coords: Vector2i` — column/row pair for `Sprite2D.frame_coords`
- Idle: cycles through 4 directions (down → up → left → right → ...) every 0.5s
...
## Test Coverage
18 tests in `tests/specs/animation_controller_test.gd`:
...
- Idle animation: direction cycling, no-change below threshold
- Walking animation: frame advance on tick, no direction cycling
- State+direction combos

## Related
- `PlayerMovement` — consumes this model for sprite animation
