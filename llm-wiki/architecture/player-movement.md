...
## Overview
Click-to-move Sprite2D view. Consumes `AnimationController` model for all frame calculations. Handles position interpolation, destination marker visibility, and direction-from-vector math.
...
`PlayerMovement` manages sprite frame rendering, position interpolation, and marker visibility — all view concerns. It contains no business logic.
