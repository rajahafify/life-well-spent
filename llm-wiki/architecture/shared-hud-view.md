...
- The Options button sits below Inventory and opens a modal with `End Game` and `Close`; pressing `End Game` emits `end_game_requested` and lets the active scene decide how to route.
...
- `tests/specs/shared_hud_view_test.gd` covers scene structure, Life updates, quest rendering, Inventory button, Options modal, End Game signal emission, `I` key toggle, echo-key rejection, equipment button application, `equipment_changed`, shortcut bar rendering, visible shortcut styling, shortcut number mapping, and `1` key signal emission.
- `tests/specs/field_scene_test.gd` covers Field using the shared HUD, blocking Options clicks from world movement, handling Apple shortcut consumption, and routing Options End Game to Main Menu.
- `tests/specs/town_scene_dialog_test.gd` covers Town using the shared HUD and routing Options End Game to Main Menu.
