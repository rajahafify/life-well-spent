...
- Step 3 completes the side chain, grants `swordsman_certification`, unlocks `swordsman_guild`, and leaves `PlayerStats.game_over_requested` true via Max Life reaching zero.

## Test Coverage
- `tests/specs/progression_model_test.gd`: XP award, linked quest completion, facility unlock once, inactive certification rejection, certification steps 1-3, Guild unlock, and game-over request.
